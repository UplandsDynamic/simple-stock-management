...
def stock_taker(stock_data: list) -> bool:
        ...
        try:
                ...
                for item in stock_data:
                    try:
                        ...
                        unrecorded_history = item.units_total - \
                            ((item.opening_stock + item.xferred_units) - item.sold_units - item.shrinkage)
                ...
                try:
                    ...
                    with transaction.atomic():
                        for item in report_stats:
                            AccountStockTakeLines(stock_take=stock_take_record, **item).save()
                    # update accountdata
                    AccountData.objects.update_or_create(defaults={
                        'owner': user,
                        'all_time_total_xfer_value': new_all_time_total_xfer_value,
                    })
                    # delete lines with zero units from database
                    AccountStockData.objects.filter(owner=user, units_total=0).delete()
                except IntegrityError as e:
                        logger.error(f'Database update failed')
                        raise
            success = True
            ...
            raise
            logger.error(f'An error has occurred during the stock taking process: {e}')
